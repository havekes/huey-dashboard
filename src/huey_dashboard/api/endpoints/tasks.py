...
def get_huey_service(
    huey=Depends(get_huey_client), db: TaskDatabase = Depends(get_task_db)
) -> HueyService:
    return HueyService(huey, db)


@router.get("/", response_model=list[TaskInfo])
async def list_tasks(service: HueyService = Depends(get_huey_service)):
    return service.list_tasks()


@router.get("/{task_id}", response_model=TaskInfo)
async def get_task(task_id: str, service: HueyService = Depends(get_huey_service)):
