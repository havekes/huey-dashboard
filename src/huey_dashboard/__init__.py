...
from .services.huey_service import poll_huey_updates
...
def init_huey_dashboard(
    ...
    # Note: FastApi usually uses 'redis' as sync for storage, but we need async for PubSub.
    # For now, let's assume 'redis' passed here might be the async client.
    async_redis: Optional[AsyncRedis] = redis if isinstance(redis, AsyncRedis) else None
    ...
    if bind_signals:
        ...
        # This will work if the FastAPI app and Huey consumer share the database and redis.
